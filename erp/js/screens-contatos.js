...
/* "Online agora" (pedido do Matt 2026-09-04: "quero saber quem esta
   online agora") — compara portal_last_active_at (heartbeat de até 2 em
   2 min, ver startActivityHeartbeat em js/portal-05-cutlist.js) contra
   uma janela de 5 min. Não é presença em tempo real (websocket/presence
   channel) — é "abriu o portal e a aba ainda estava em primeiro plano
   há pouco". Pra este uso (saber quem está mexendo no sistema agora)
   é suficiente e não precisa de infra nova. */
ScreensContatos.ONLINE_JANELA_MS = 5 * 60 * 1000;
ScreensContatos.online = function (ts) {
  if (!ts) return false;
  const d = new Date(ts);
  if (isNaN(d)) return false;
  return (Date.now() - d.getTime()) < ScreensContatos.ONLINE_JANELA_MS;
};

/* 'Hoje' comparando só a data (sem hora), do jeito mais simples que combina
   com UI.date: os dois pegam os 10 primeiros caracteres do ISO (a data
   crua, sem converter fuso) em vez de instanciar Date e formatar. */
ScreensContatos.ehDeHoje = function (ts) {
  if (!ts) return false;
...
ScreensContatos.lista = function (params, d) {
  ...
  const lista = (filtro === 'todos' ? todos
    ...
    /* Novos em cima: mais recente primeiro pela data de abertura (created_at).
       .slice() porque Array.sort muta — sem isso a ordenação vazaria pra
       'todos' também (mesma referência quando filtro === 'todos'). */
    .slice()
  ...
  const online = todos.filter(function (c) { return c.conta && ScreensContatos.online(c.conta.portal_last_active_at); }).length;
    ...
    return {
      ...
      acesso: c.conta
        ? (c.conta.ativa === false ? UI.pill('acesso suspenso', 'erp-pill-danger') + ' ' : '') +
          (ScreensContatos.online(c.conta.portal_last_active_at) ? UI.pill('online agora', 'erp-pill-ok') + ' ' : '') +
      ...
      desde: (ScreensContatos.ehDeHoje(c.created_at)
        ? '<span class="erp-pill erp-pill-ok erp-xs">NEW</span> '
  ...
  return UI.head('Central de Contatos',
    ...
    UI.kpi('Online agora', online, online ? 'nos últimos 5 min' : 'ninguém no portal agora') +
...
ScreensContatos._painelAcesso = function (c, d) {
  ...
  return UI.panel('Acesso ao sistema',
    ...
    (ScreensContatos.online(conta.portal_last_active_at) ? UI.pill('online agora', 'erp-pill-ok') : '') +
