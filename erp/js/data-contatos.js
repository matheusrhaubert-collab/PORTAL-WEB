...
CT.load = async function () {
  ...
  const [contatosRes, papeisRes, contasRes, perfisRes, profilesRes] = await Promise.all([
    ...
    DATA.sb().from('user_profiles').select('user_id, role, logo_url, last_active_at')
  ]);
  ...
  const contatos = (contatosRes.data || []).map(function (c) {
    ...
    if (c.conta && c.conta.user_id && profilesByUser[c.conta.user_id]) {
      ...
      // last_active_at é o heartbeat do portal (js/portal-05-cutlist.js,
      // touchLastActive/startActivityHeartbeat) — "online agora" em
      // screens-contatos.js compara isto contra um limite de minutos,
      // diferente de ultimo_acesso (que é só o instante do LOGIN).
      c.conta.portal_last_active_at = profilesByUser[c.conta.user_id].last_active_at;
    }
    return c;
  });
