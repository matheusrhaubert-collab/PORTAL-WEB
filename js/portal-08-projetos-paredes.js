...
function closeProjectReposicionarModal(commit) {
  stopProjectReposicionarViewDrag(); // nunca deixa o listener de document sobrevivendo ao fechar o painel
...
// Estado do arraste na VISTA 2D — guardado FORA da função de render (mesmo
// padrão de projectReposicionarStepMm acima) porque o innerHTML inteiro do
// painel é recriado a cada applyValues (pra refletir o valor clampado nos
// campos numéricos), o que destruiria qualquer listener puramente local —
// os listeners de arraste ficam no `document`, sobrevivendo à recriação do
// SVG, e são sempre limpos ao soltar o botão OU ao fechar o painel (ver
// closeProjectReposicionarModal acima).
let projectReposicionarViewDrag = null;

function stopProjectReposicionarViewDrag() {
  if (!projectReposicionarViewDrag) return;
  document.removeEventListener('pointermove', projectReposicionarViewDrag.onMove);
  document.removeEventListener('pointerup', projectReposicionarViewDrag.onUp);
